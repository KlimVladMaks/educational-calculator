...
class CalendarsFrame(BaseFrame):
    ...
    def create_frame(self):
        self.back_button = ttk.Button(
            self.master, text="Назад", command=self.go_back)
    ...
    def get_table_data(self):
        ...
        for data in table_data:
            total_days = Calculator.count_days_between_dates(data[1], data[2])
            days_off = len(data[3])
    ...
    def download_calendar(self):
        ...
        if file_path:
            with open(file_path, 'r', encoding='utf-8') as file:
                data = json.load(file)
                new_calendar_data = [
                    data["name"], data["start_date"], data["end_date"], data["days_off_list"]
                ]
